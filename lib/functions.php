...
function add_item($item_id, $user_id, $quantity, $cost)
{
    ...
    $db = getDB();
    $stmt = $db->prepare("INSERT INTO Cart (product_id, user_id, quantity, unit_cost) VALUES (:iid, :uid, :q, :uc) ON DUPLICATE KEY UPDATE quantity = quantity + :q");
    try {
        ...
        $stmt->bindValue(":uc", $cost, PDO::PARAM_INT);
        $stmt->execute();
        return true;
    } catch (PDOException $e) {
